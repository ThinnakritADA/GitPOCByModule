...
class Printbarcode_Model extends CI_Model
{
    function FSxMPriBarMoveDataIntoTable($paData)
    {
        ...
        $tPrnBarBrowseBchFromCode           = $paData['aDataWhere']['tPrnBarBrowseBchFromCode'];
        $tPrnBarBrowseBchToCode             = $paData['aDataWhere']['tPrnBarBrowseBchToCode'];
        $tPrnBarBrowseGrpPriFromCode        = $paData['aDataWhere']['tPrnBarBrowseGrpPriFromCode'];
        ...
        // if ($bSeleteImport == 0) {
            ...
            if( $tPriType == "ALL" ){
                $tPri4PDT = "   SELECT FTPdtCode,FTPunCode,MAX(FTPghDocNo) AS FTPghDocNo FROM TCNTPdtPrice4PDT WITH(NOLOCK)
                                WHERE FTPghDocType = '1' ".$tWherePri." AND FTPplCode = '$tPrnBarBrowseGrpPriFromCode'
                                GROUP BY FTPdtCode,FTPunCode

                                UNION

                                SELECT FTPdtCode,FTPunCode,MAX(FTPghDocNo) AS FTPghDocNo FROM TCNTPdtPrice4PDT WITH(NOLOCK)
                                WHERE FTPghDocType = '2' ".$tWherePri." AND FTPplCode = '$tPrnBarBrowseGrpPriFromCode'
                                GROUP BY FTPdtCode,FTPunCode ";
            }else{
                $tPri4PDT = "   SELECT FTPdtCode,FTPunCode,MAX(FTPghDocNo) AS FTPghDocNo
                                FROM TCNTPdtPrice4PDT WITH(NOLOCK)
                                WHERE FTPghDocType = '".$tPriType."' ".$tWherePri." AND FTPplCode = '$tPrnBarBrowseGrpPriFromCode'
                                GROUP BY FTPdtCode,FTPunCode ";
            }

            if($tPRNLblCode == 'SL001'){ // กรณีเป็นรูปแบบการพิมพ์ของ สล.แลนด์
                $tPlbPriPerUnit = " SELECT 
                                        CASE WHEN(LEN(PUF.FCPdtUnitFact) - LEN(REPLACE(PUF.FCPdtUnitFact, '|', ''))) = 0 THEN PUF.FCPdtUnitFact + '| |' 
                                        ELSE 
                                            CASE WHEN (LEN(PUF.FCPdtUnitFact) - LEN(REPLACE(PUF.FCPdtUnitFact, '|', ''))) = 1 THEN PUF.FCPdtUnitFact + '|'
                                            ELSE PUF.FCPdtUnitFact 
                                            END 
                                        END AS FCPdtUnitFact
                                    FROM (
                                        SELECT STUFF((
                                            SELECT TOP 3 '|' + PUN.FTPunName + ';' + 
                                                CONVERT(VARCHAR, CAST(ISNULL(P.FCPgdPriceRet, 0) AS NUMERIC(18, 2))) + '/' + 
				                                CONVERT(VARCHAR, CAST(ISNULL((P.FCPgdPriceRet / NULLIF(PPS.FCPdtUnitFact, 0)), 0) AS NUMERIC(18, 2)))
                                            FROM TCNMPdtPackSize PPS WITH(NOLOCK) 
                                            INNER JOIN TCNMPdtUnit_L PUN WITH(NOLOCK) ON PPS.FTPunCode = PUN.FTPunCode AND PUN.FNLngID = 1 
                                            INNER JOIN (
                                                SELECT ROW_NUMBER() OVER(PARTITION BY PRI.FTPdtCode, PRI.FTPunCode ORDER BY PRI.FDCreateOn DESC) AS RowID, PRI.FCPgdPriceRet, PRI.FTPdtCode, PRI.FTPunCode
                                                FROM TCNTPdtPrice4PDT PRI WITH (NOLOCK)
                                                WHERE PRI.FDPghDStart <= CONVERT (VARCHAR(10), GETDATE(), 121) AND PRI.FDPghDStop >= CONVERT (VARCHAR(10), GETDATE(), 121) 
                                                    AND PRI.FTPghTStart <= CONVERT(time,GETDATE()) AND PRI.FTPghTStop >= CONVERT(time,GETDATE()) AND PRI.FTPghDocType = 1
                                                    AND PRI.FTPplCode = '$tPrnBarBrowseGrpPriFromCode'
                                            ) P ON P.FTPdtCode = PPS.FTPdtCode AND P.FTPunCode = PPS.FTPunCode AND P.RowID = 1
                                            WHERE PPS.FTPdtCode = PDT.FTPdtCode
                                            ORDER BY PPS.FCPdtUnitFact, PPS.FDCreateOn ASC
                                            FOR XML PATH('')), 1, 1, '') AS FCPdtUnitFact ) PUF ";

                $tJoinTCNMPdtBar = " INNER JOIN (SELECT 
                                        ROW_NUMBER() OVER(PARTITION BY FTPdtCode, FTPunCode ORDER BY FTPdtCode, FTPunCode ASC) AS rowID, FTPdtCode, FTBarCode, FTPunCode
                                        FROM TCNMPdtBar WITH (NOLOCK)
                                    ) BAR ON BAR.FTPdtCode = PPS.FTPdtCode AND BAR.FTPunCode = PPS.FTPunCode AND rowID = '1' ";
            }else{ // รูปแบบการพิมพ์ STD
                $tPlbPriPerUnit = " SELECT TOP 1 CONVERT(VARCHAR,CAST(PRI.FCPgdPriceRet AS NUMERIC(18,".$nOptDecimalShow."))) + '/' + CONVERT(VARCHAR,CAST(PPS.FCPdtUnitFact AS NUMERIC(18,".$nOptDecimalShow."))) + ' ' + PUN.FTPunName 
                                    FROM TCNMPdtPackSize PPS WITH(NOLOCK)
                ...
                $tJoinTCNMPdtBar = " INNER JOIN TCNMPdtBar BAR WITH(NOLOCK) ON BAR.FTPdtCode = PPS.FTPdtCode AND BAR.FTPunCode = PPS.FTPunCode ";
            }
            ...
            $tSQLSelect = "     INSERT INTO TCNTPrnLabelTmp (FTComName,FTPdtCode,FTPdtName,FTBarCode,FCPdtPrice,FTPlcCode,FDPrnDate,
                                ...
                                ,FTPdtNameOth,FTPlbSubDept,FTPlbSellingUnit,FCPdtOldPrice,FTPlbCapFree,FTPlbPdtChain,FTPlbClrName,FTPlbPszName,FDPlbPmtDStart,FDPlbPmtDStop,FTPlbPriPerUnit,FCPdtUnitFact) ";
            ...
            $tSQLSelect .= $tPlbPriPerUnit;

            $tSQLSelect .= "        ),'') AS FTPlbPriPerUnit,
                                    CONVERT ( VARCHAR, CAST ( PPS.FCPdtUnitFact AS NUMERIC ( 18, 2 ) ) ) AS FCPdtUnitFact
                                    /* END CASE STD */
            ...
            $tSQLSelect .= $tJoinTCNMPdtBar;

            $tSQLSelect .= "    LEFT JOIN TCNMPdtUnit_L PUL WITH(NOLOCK) ON PUL.FTPunCode = PPS.FTPunCode AND PUL.FNLngID = ".$nLangUnit."
                                ...
                                LEFT JOIN ( 
                                    ...
                                    INNER JOIN ( 
                                        SELECT FTPdtCode,FTPunCode,MAX(FTPghDocNo) AS FTPghDocNo FROM TCNTPdtPrice4PDT WITH(NOLOCK)
                                        WHERE FTPghDocType = '1' AND FTPplCode = '$tPrnBarBrowseGrpPriFromCode'
                                        GROUP BY FTPdtCode,FTPunCode
                                    ) PRI2 ON PRI2.FTPdtCode = PRI.FTPdtCode AND PRI2.FTPunCode = PRI.FTPunCode AND PRI2.FTPghDocNo = PRI.FTPghDocNo
                                ...
                                LEFT JOIN TCNTPdtPrice4PDT PP4PDT WITH(NOLOCK) ON PP4PDT.FTPdtCode = Pdt.FTPdtCode AND PP4PDT.FTPunCode = BAR.FTPunCode
            ...
            //กรองสาขา
            if ($tPrnBarBrowseBchFromCode != '' && $tPrnBarBrowseBchToCode != '') {
                $tSQLSelect .= " AND (PSB.FTBchCode Between  '$tPrnBarBrowseBchFromCode' AND '$tPrnBarBrowseBchToCode' OR PSB.FTBchCode Between  '$tPrnBarBrowseBchToCode' AND '$tPrnBarBrowseBchFromCode') ";
            }
            ...
            //กรองกลุ่มราคาสินค้า
            if ($tPrnBarBrowseGrpPriFromCode != '') {
                $tSQLSelect .= " AND PP4PDT.FTPplCode = '$tPrnBarBrowseGrpPriFromCode' ";
            }
